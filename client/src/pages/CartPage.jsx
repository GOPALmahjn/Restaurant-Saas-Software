      ...
      <div className="space-y-4">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 flex items-center gap-1.5 text-sm text-[#A0AEC0]">
        ...
        {cart.length === 0 ? (
          <motion.div
            initial={{ opacity: 0, y: 16 }}
            animate={{ opacity: 1, y: 0 }}
            className="flex flex-col items-center gap-4 rounded-[28px] border border-dashed border-white/10 bg-white/5 p-14 text-center"
          >
            <span className="flex h-20 w-20 items-center justify-center rounded-3xl border border-white/10 bg-white/5 text-3xl">
              🍽️
            </span>
            <div>
            ...
            <AnimatePresence initial={false}>
              {cart.map((entry) => (
                <motion.li
                  ...
                  className="flex flex-col gap-4 rounded-3xl border border-white/10 bg-white/5 p-4 backdrop-blur-xl transition-colors hover:border-white/20 sm:flex-row sm:items-center sm:justify-between sm:p-5"
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="min-w-0">
                      <Link to={`/menu/${entry.id}`}>
                        <h2 className="truncate font-semibold text-white transition-colors hover:text-[#FF6B35]">
                          {entry.name}
                        </h2>
                      </Link>
                      <p className="mt-0.5 text-sm text-[#A0AEC0]">${entry.price.toFixed(2)} each</p>
                      {entry.type && (
                        <div className="mt-2">
                  ...
                  <div className="flex items-center justify-between gap-4 sm:justify-end">
                    <div className="flex items-center gap-1 rounded-full border border-white/10 bg-[#070B16]/60 p-1">
                      <motion.button
                        type="button"
                        whileTap={{ scale: 0.85 }}
                        onClick={() => updateQuantity(entry.id, entry.quantity - 1)}
                        aria-label={`Decrease ${entry.name} quantity`}
                        className="inline-flex h-8 w-8 items-center justify-center rounded-full text-[#A0AEC0] transition-colors hover:bg-white/10 hover:text-white"
                      >
                        <Minus size={14} />
                      </motion.button>
                    ...
                    <motion.button
                      ...
                      className="inline-flex h-9 w-9 items-center justify-center rounded-full text-[#A0AEC0] transition-colors hover:bg-red-500/12 hover:text-red-400"
                    >
                      <Trash2 size={15} />
                    </motion.button>
                  </div>
      ...
      >
        ...
        <dl className="mt-6 space-y-3 text-sm">
          ...
          <Row
            ...
            accent={discount > 0 ? 'text-emerald-400' : 'text-[#A0AEC0]'}
          />
        ...
        {/* Coupon */}
        <div className="mt-6 rounded-2xl border border-white/10 bg-[#070B16]/50 p-4">
          <label
            htmlFor="coupon"
            className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-[#A0AEC0]"
          >
          ...
          <div className="mt-2.5 flex items-center gap-2">
            <input
              ...
              className="w-full rounded-full border border-white/10 bg-[#070B16]/60 px-4 py-2.5 text-sm text-white outline-none transition-colors placeholder:text-[#A0AEC0]/60 focus:border-[#FF6B35]/50"
            />
            <AnimatePresence>
              {discount > 0 && (
                <motion.span
        ...
        <motion.div whileHover={{ y: -2 }} whileTap={{ scale: 0.98 }} className="mt-6">
          <Link
            ...
            className={`group inline-flex w-full items-center justify-center gap-2 rounded-full px-5 py-4 font-semibold transition-all ${
              cart.length === 0
                ? 'pointer-events-none bg-white/10 text-[#A0AEC0]'
